function [DEM] = ROS_spm_ADEM(DEM, postarget)
...
for iE = 1:nE
    ...
    for iY = 1:nY
        ...
        for i = 1: size(LinkNames,1)
            ...
            X1=sumresp.PoseStamped.Pose.Position.X;
            Y1=sumresp.PoseStamped.Pose.Position.Y;
            Z1=sumresp.PoseStamped.Pose.Position.Z;
            ALL(:,i) = [X1;Y1;Z1];
        end
        
        
        PoseEndEffector = ALL(:,end); 
        plot3(PoseEndEffector(1), PoseEndEffector(2), PoseEndEffector(3),'-rx')
        T = PosEndEffectorPR2(postarget);

        hold on
        plot3(T(1),T(2),T(3),'ob','Markersize',10)
        hold on  
